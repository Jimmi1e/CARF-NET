...
class HybridPatchMatchNet(PatchmatchNet):
    """
    HybridPatchMatchNet 在原 PatchmatchNet 的基础上增加了 GoMVS 分支，
    先调用父类 PatchmatchNet 的 forward() 得到初步深度 (pm_depth)，
    然后使用相同的 FeatureNet 提取全分辨率特征，
    将这些特征以及 pm_depth 传递给 GoMVS 分支进行深度优化，
    最后简单平均两者得到最终深度。
    """
    def __init__(self,
                 patchmatch_interval_scale: List[float],
                 ...
                 Attention_Selection='None',
                 image_size=(512,512)):
        super(HybridPatchMatchNet, self).__init__(patchmatch_interval_scale, propagation_range,
                                                    patchmatch_iteration, patchmatch_num_sample,
                                                    propagate_neighbors, evaluate_neighbors,
                                                    featureNet, Attention_Selection, image_size)
        self.gomvs = GoMVS()  # 使用默认参数
    ...
    def forward(self,
                ...
                extrinsics: torch.Tensor,
                depth_min: torch.Tensor,
                depth_max: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[int, List[torch.Tensor]]]:
        # 如果 images 是 tensor，则转换为列表
        if isinstance(images, torch.Tensor):
            images = [images[:, i] for i in range(images.shape[1])]
        
        # 1. 调用原 PatchmatchNet 的 forward() 得到初步深度 (pm_depth) 以及中间结果
        pm_depth, photometric_conf_pm, depth_patchmatch = super(HybridPatchMatchNet, self).forward(images, intrinsics, extrinsics, depth_min, depth_max)
        # pm_depth: [B, 1, H_pm, W_pm] (低分辨率初步深度)
        
        # 2. 共享提取全分辨率特征（使用 FeatureNet 提取 stage3 特征）
        features = []
        for img in images:
            feat = self.feature(img)
            features.append(feat)
        ref_feature_full = features[0][3]           # 参考视角全分辨率特征 [B, C, H_full, W_full]
        src_features_full = [f[3] for f in features[1:]]  # 源视角全分辨率特征
        
        # 3. 构造全分辨率的投影矩阵（假设传入的 extrinsics 适用于全分辨率）
        ref_proj = extrinsics[:, 0].unsqueeze(1)  # [B, 1, 4, 4]
        
        # 4. 调用 GoMVS 分支
        # 注意，这里传入的 intrinsics 为参考视角的内参
        outputs_gomvs, _ = self.gomvs(ref_feature_full, src_features_full, ref_proj, None, pm_depth, intrinsics[:,0], None)
        depth_gomvs = outputs_gomvs["depth"]
        
        # 5. 将 PatchMatch 得到的初步深度上采样到全分辨率
        depth_pm_up = F.interpolate(pm_depth, size=ref_feature_full.shape[2:], mode='bilinear', align_corners=False)
        
        # 6. 融合两条分支的深度（简单平均）
        depth_final = (depth_pm_up + depth_gomvs) / 2.0
        
        return depth_final, photometric_conf_pm, depth_patchmatch
