...
class GoConv3D(nn.Module):
    ...
    def get_gcacost(self, x_ori, dp, normal, intri):
        # 确保 intri 的形状为 [B, 3, 3]
        if intri.dim() == 4:
            intri = intri.squeeze(1)  # 将形状 [B, 1, 3, 3] 变为 [B, 3, 3]
        
        # 为便于后续广播，将 fx, fy, cx, cy 扩展为 [B, 1]
        fx = intri[:, 0, 0].unsqueeze(1)  # [B,1]
        fy = intri[:, 1, 1].unsqueeze(1)  # [B,1]
        cx = intri[:, 0, 2].unsqueeze(1)  # [B,1]
        cy = intri[:, 1, 2].unsqueeze(1)  # [B,1]
        ...
        normal_p_unfold = normal_p_unfold.view(b, 3, kernel_size ** 2, h, w).squeeze(1)  # [B, 3, k2, h, w]
        ...
        w_ori = w * stride
        y, x = torch.meshgrid([
            torch.arange(0, h_ori, dtype=torch.float32, device=normal.device),
            torch.arange(0, w_ori, dtype=torch.float32, device=normal.device),
        ], indexing="ij")
        y, x = y.contiguous().view(h_ori * w_ori), x.contiguous().view(h_ori * w_ori)
        xy = torch.stack((x, y))  # [2, H*W]
        xy = xy.unsqueeze(0).repeat(b, 1, 1)  # [B, 2, H*W]

        # 对于广播，cx, fx 等为 [B, 1]，这样 (xy[:,0,:] - cx) 得到 [B, H*W]
        posx = (xy[:, 0, :] - cx) / fx  # [B, H*W]
        posy = (xy[:, 1, :] - cy) / fy  # [B, H*W]
        ...
        ddw_num = nx * pos_u_center + ny * pos_v_center + nz  # [B, k2, h, w]
        ddw_denom = nx * pos_u + ny * pos_v + nz  # [B, k2, h, w]
        ddw_denom[torch.abs(ddw_denom) < 1e-8] = torch.sign(ddw_denom[torch.abs(ddw_denom) < 1e-8]) * 1e-8

        ddw_weights = ddw_num / ddw_denom  # [B, k2, h, w]
  ...
  # [1, 2, H*W] reshape to [B, 2, H, W] later if needed

        xy_p = F.pad(xy_n, pad=[pad, pad, pad, pad], mode='replicate')
        xy_p = F.unfold(xy_p, [kernel_size, kernel_size], padding=0, stride=stride)
        xy_p = xy_p.reshape(b, 2, kernel_size**2, h, w).unsqueeze(1).repeat(1, d, 1, 1, 1, 1)
