    ...
    def forward(self, features):
        ...
        for nview_idx, feature_multi_stages in enumerate(features):
            if nview_idx == 0: # ref view
                ref_fea_t_list = self.FMT(feature_multi_stages[3].clone(), feat="ref")
                feature_multi_stages[3] = ref_fea_t_list[-1]
                feature_multi_stages[2] = self.smooth_1(self._upsample_add(self.dim_reduction_1(feature_multi_stages[3]), feature_multi_stages[2]))
                feature_multi_stages[1] = self.smooth_2(self._upsample_add(self.dim_reduction_2(feature_multi_stages[2]), feature_multi_stages[1]))

            else: # src view
                feature_multi_stages[3] = self.FMT([_.clone() for _ in ref_fea_t_list], feature_multi_stages[3].clone(), feat="src")
                feature_multi_stages[2] = self.smooth_1(self._upsample_add(self.dim_reduction_1(feature_multi_stages[3]), feature_multi_stages[2]))
                feature_multi_stages[1] = self.smooth_2(self._upsample_add(self.dim_reduction_2(feature_multi_stages[2]), feature_multi_stages[1]))
